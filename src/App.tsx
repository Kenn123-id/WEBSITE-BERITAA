...
import { Article, Comment, LogEntry, Role, User, RegisteredStaff } from './types';
import { 
  INITIAL_ARTICLES, 
  INITIAL_COMMENTS, 
  INITIAL_LOGS 
...
export default function App() {
  ...
  // Registered staff accounts list with approval state
  ...
  const activePublisherRecord = currentUser && currentUser.role === 'publisher'
    ? registeredStaff.find((s) => s.username.toLowerCase() === currentUser.username.toLowerCase())
    : null;

  const isPublisherApproved = activePublisherRecord ? (activePublisherRecord.approved ?? false) : true;

  const activeDeveloperRecord = currentUser && currentUser.role === 'developer'
    ? registeredStaff.find((s) => s.username.toLowerCase() === currentUser.username.toLowerCase())
    : null;

  const isDeveloperApproved = activeDeveloperRecord ? (activeDeveloperRecord.approved ?? false) : true;
  ...
  const handleToggleStaffApproval = (username: string) => {
    setRegisteredStaff((prev) =>
      prev.map((staff) => {
        if (staff.username.toLowerCase() === username.toLowerCase()) {
          const newApproved = !staff.approved;
          const statusTxt = newApproved ? 'AKTIF (DI-ACC)' : 'PENDING (DICABUT)';
          logSystemEvent(
            'info',
            `Status persetujuan akun staf "${staff.name}" (${staff.role}) diubah menjadi: ${statusTxt}.`,
            'Manajer Staf'
          );
          triggerToast(`Akun ${staff.name} berhasil di-change ke ${newApproved ? 'Aktif' : 'Pending'}.`);
          return { ...staff, approved: newApproved };
        }
        return staff;
      })
    );
  ...
  const handleLoginSuccess = (role: 'publisher' | 'developer', name: string, username: string) => {
    const newUser: User = { username, role, name };
    setLoggedUsers((prev) => ({
      ...prev,
      [role]: newUser
    }));
          ...
          {activeRole === 'publisher' && (
            <div key="publisher-panel">
              <PublisherDashboard
                ...
                isApproved={isPublisherApproved}
          ...
          {activeRole === 'developer' && (
            <div key="developer-panel">
              <DeveloperDashboard
                ...
                onToggleStaffApproval={handleToggleStaffApproval}
                isApproved={isDeveloperApproved}
                autoPilotEnabled={autoPilotEnabled}
                setAutoPilotEnabled={setAutoPilotEnabled}
                autoPilotInterval={autoPilotInterval}
                setAutoPilotInterval={setAutoPilotInterval}
        ...
        {showLoginModal && (
          <div key="login-dialog-wrap">
            <LoginPage
              ...
              registeredStaff={registeredStaff}
              setRegisteredStaff={setRegisteredStaff}
            />
          </div>
        )}
      </AnimatePresence>
