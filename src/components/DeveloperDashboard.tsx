...
import { LogEntry, Article, Comment, RegisteredStaff } from '../types';
import { 
  Terminal, 
  Settings, 
  Database, 
...
interface DeveloperDashboardProps {
  ...
  onToggleStaffApproval: (username: string) => void;
  isApproved: boolean;
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all cursor-pointer ${
                    activeTab === tab
                      ? 'bg-white dark:bg-zinc-700 text-gray-950 dark:text-white shadow-xs font-bold'
                      : 'text-gray-550 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-zinc-200'
                  }`}
                >
                  {labels[tab]}
                </button>
  ...
  isApproved: boolean; // Persetujuan moderasi penerbit
      ...
      {activeTab === 'approval' && (
        <div className="space-y-6" id="dev-view-approval">
          {/* Sub-tab approval navigator */}
          <div className="flex border-b border-gray-150 dark:border-zinc-800 gap-6">
            <button
              onClick={() => setApprovalSubTab('news')}
            ...
            >
              ...
              {articles.filter((art) => art.status === 'pending').length > 0 && (
                <span className="bg-rose-500 text-white text-[10px] px-1.5 py-0.5 rounded-full font-extrabold shrink-0">
                  {articles.filter((art) => art.status === 'pending').length}
                </span>
              )}
            </button>
            <button
              ...
              className={`pb-3 text-sm font-bold border-b-2 transition-all cursor-pointer flex items-center gap-2 ${
                approvalSubTab === 'staff'
                  ? 'border-indigo-600 text-indigo-600 dark:border-indigo-400 dark:text-indigo-400'
                  : 'border-transparent text-gray-400 hover:text-gray-650 dark:hover:text-zinc-300'
              }`}
            >
              <span>Persetujuan Akun Staf</span>
              {registeredStaff.filter((s) => s.role === 'publisher' && !s.approved).length > 0 && (
                <span className="bg-amber-500 text-white text-[10px] px-1.5 py-0.5 rounded-full font-extrabold shrink-0 animate-pulse">
                  {registeredStaff.filter((s) => s.role === 'publisher' && !s.approved).length}
                </span>
              )}
            </button>
          </div>
          ...
          {approvalSubTab === 'staff' && (
            <div className="space-y-4" id="staff-approval-block">
              <div className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-2xl p-5 shadow-xs">
                <h3 className="text-base font-bold text-gray-900 dark:text-white mb-1">
                  Kontrol Persetujuan & Aktivasi Akun Staf
                </h3>
                <p className="text-xs text-gray-400">
                  Berikut adalah daftar akun staf (Penerbit/Publisher & Pengembang/Developer kustom) yang terdaftar di portal. Anda dapat melakukan aktivasi (ACC) instan agar mereka dapat mengakses dasbor penuh atau mencabut aksesnya demi keamanan portal.
                </p>
              </div>

              <div className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-2xl overflow-hidden divide-y divide-gray-100 dark:divide-zinc-800/60 shadow-xs">
                {registeredStaff.length === 0 ? (
                  <div className="p-10 text-center space-y-2">
                    <p className="text-xs text-gray-400 font-semibold">Belum ada akun kustom staf terdaftar.</p>
                    <p className="text-[11px] text-gray-500">Gunakan fitur Register di formulir masuk untuk mendaftarkan akun staf baru.</p>
                  </div>
                ) : (
                  registeredStaff
                    .map((staff) => (
                      <div key={staff.username} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="space-y-1">
                          <h4 className="text-sm font-bold text-gray-950 dark:text-white flex items-center gap-2">
                            <span>{staff.name}</span>
                            <span className="text-[10px] font-mono text-gray-400">(@{staff.username})</span>
                          </h4>
                          <p className="text-xs text-gray-400 flex items-center gap-1.5">
                            Role: <strong className={staff.role === 'developer' ? "text-indigo-600 dark:text-indigo-400 font-extrabold uppercase text-[10px]" : "text-emerald-600 dark:text-emerald-400 font-extrabold uppercase text-[10px]"}>{staff.role === 'developer' ? 'Pengembang (Developer)' : 'Penerbit (Publisher)'}</strong> | Sandi: <span className="font-mono bg-gray-100 dark:bg-zinc-950 px-1 py-0.5 rounded text-[10px]">{staff.password || '******'}</span>
                          </p>
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                          <span className={`text-xs px-2.5 py-1 rounded-full font-bold uppercase tracking-wider ${
                            staff.approved
                              ...
                              : 'bg-amber-100 dark:bg-amber-950/40 text-amber-800 dark:text-amber-400 animate-pulse'
                          }`}>
                            {staff.approved ? '🟢 AKTIF (DI-ACC)' : '⏳ MENUNGGU ACC'}
                          </span>

                          <button
                            onClick={() => onToggleStaffApproval(staff.username)}
                            className={`px-4 py-2 text-xs font-bold rounded-xl transition-all cursor-pointer flex items-center gap-1.5 shadow-sm active:scale-95 ${
                              staff.approved
                                ? 'bg-rose-50 hover:bg-rose-100 text-rose-700 dark:bg-rose-950/20 dark:text-rose-450 dark:hover:bg-rose-900/30'
                                : 'bg-indigo-600 hover:bg-indigo-700 text-white dark:bg-indigo-650 dark:hover:bg-indigo-600 shadow-md shadow-indigo-600/10'
                            }`}
                          >
                            {staff.approved ? (
                              <>
                              ...
                              <>
                                ...
                                <span>BERI ACC / AKTIFKAN Akun</span>
                              </>
                            )}
                          </button>
                        </div>
