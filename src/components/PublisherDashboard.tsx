...
export default function PublisherDashboard({
  ...
  // Dismissible approval notification state
  const [showApprovedNotif, setShowApprovedNotif] = useState(() => {
    return localStorage.getItem(`portal_dismissed_approved_notif_${publisherName}`) !== 'true';
  });

  // Automatically switch tabs if account status changes to unapproved
  React.useEffect(() => {
    if (!isApproved && (activeTab === 'create' || activeTab === 'comments')) {
  ...
  const handleDismissNotif = () => {
    localStorage.setItem(`portal_dismissed_approved_notif_${publisherName}`, 'true');
    setShowApprovedNotif(false);
  };
    ...
    <div id="publisher-dashboard" className="py-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-gray-100 dark:border-zinc-800 pb-5 gap-4">
        ...
        <div className="flex items-center gap-1.5 p-1 bg-gray-100 dark:bg-zinc-850 rounded-xl max-w-max self-start" id="pub-tabs">
          {(isApproved ? (['analytics', 'create', 'articles', 'comments'] as const) : (['analytics', 'articles'] as const)).map((tab) => {
            const labels = {
      ...
      {/* ⚠️ NOTIFIKASI AKTIVASI AKUN / PERSETUJUAN */}
      <AnimatePresence>
        {!isApproved && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="p-5 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/65 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm"
          >
            <div className="space-y-1">
              <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-extrabold tracking-wider bg-amber-100 dark:bg-amber-950 text-amber-805 dark:text-amber-400 uppercase animate-pulse">
                Menunggu ACC Developer
              </span>
              <h4 className="text-sm font-bold text-amber-900 dark:text-amber-300">
                Status Akun Penerbit Belum Aktif
              </h4>
              <p className="text-xs text-amber-700 dark:text-amber-405 leading-relaxed">
                Akun Anda <strong>{publisherName}</strong> sedang ditinjau. Anda dapat melihat statistik & arsip rilis Anda, namun fitur pembuatan berita manual serta rilis AI autopilot saat ini terkunci.
              </p>
            </div>
        ...
        {isApproved && showApprovedNotif && publisherName !== 'Redaktur Utama' && publisherName !== 'Redaktur' && (
          <motion.div
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="p-5 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900/60 rounded-2xl flex items-start justify-between gap-4 shadow-sm"
          >
            <div className="space-y-1">
              <span className="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-extrabold tracking-wider bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-400 uppercase">
                Notifikasi Sistem
              </span>
              <h4 className="text-sm font-bold text-emerald-900 dark:text-emerald-300">
                Sesi Akun Penerbit Anda Telah Aktif! 🎉
              </h4>
              <p className="text-xs text-emerald-700 dark:text-emerald-400/90 leading-relaxed">
                Selamat! Hasil pendaftaran akun Penerbit milik <strong>{publisherName}</strong> telah disetujui secara resmi oleh tim pengembang (developer). Sekarang Anda memiliki akses tak terbatas untuk merilis berita utama KabarNusantara.
              </p>
            </div>
            <button
              onClick={handleDismissNotif}
              className="text-emerald-800 dark:text-emerald-400 hover:bg-emerald-100/60 dark:hover:bg-emerald-950 p-1.5 rounded-lg transition-colors cursor-pointer shrink-0"
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-indigo-100/40 dark:border-zinc-800/50 pb-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    disabled={!isApproved}
                    className="px-3.5 py-1.5 bg-indigo-650 hover:bg-indigo-700 disabled:bg-gray-200 disabled:dark:bg-zinc-805 disabled:text-gray-400 disabled:dark:text-zinc-600 disabled:cursor-not-allowed text-white text-xs font-bold rounded-xl shadow-xs transition-all flex items-center gap-1.5 cursor-pointer dark:bg-indigo-600 dark:hover:bg-indigo-550"
                  >
                ...
                <div className="bg-white/80 dark:bg-zinc-900/60 p-4 rounded-xl border border-gray-100/65 dark:border-zinc-900 flex flex-col justify-between space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      disabled={!isApproved}
                      className={`w-full py-2 px-4 rounded-lg font-bold text-xs transition-all cursor-pointer text-center disabled:bg-gray-200 disabled:dark:bg-zinc-805 disabled:text-gray-400 disabled:dark:text-zinc-650 disabled:cursor-not-allowed ${
                        autoPilotEnabled
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {[15, 30, 60, 180].map((sec) => (
                      <button
                        ...
                        disabled={!isApproved}
                        className={`px-2.5 py-1 text-[10px] font-bold rounded-md transition-all cursor-pointer disabled:bg-gray-250 disabled:dark:bg-zinc-805 disabled:text-gray-405 disabled:dark:text-zinc-600 disabled:cursor-not-allowed ${
                ...
                <div className="bg-white/80 dark:bg-zinc-900/60 p-4 rounded-xl border border-gray-100/65 dark:border-zinc-900 flex flex-col justify-between space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={!isApproved}
                      className={`flex-1 py-1.5 px-3 rounded-md font-bold text-[10px] text-center transition-all cursor-pointer disabled:bg-gray-250 disabled:dark:bg-zinc-805 disabled:text-gray-405 disabled:dark:text-zinc-600 disabled:cursor-not-allowed ${
